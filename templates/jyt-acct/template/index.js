//{{controllerName}}
app.controller('{%controllerName%}', ['$scope', '$http', 'hints', '$timeout', '$uibModal', '$cookies', '$location','$rootScope','queryCriteria','pagination',function($scope, $http, hints, $timeout, $uibModal,  $cookies ,$location,$rootScope,queryCriteria,pagination) {
	...
	queryCriteria.run($scope, params, sessionStorage.{%controllerName%}Params,'startTime','endTime');
	{%#query.tradeDate%}
    if(!sessionStorage.{%controllerName%}Params || $scope.params.startTime == null || $scope.params.endTime == null){
        $scope.dt = {from: {date: new Date(), opened: false}, to: {date: new Date(), opened: false}};
	}
	...
	function query() { //TODO 查询
		{%#query.tradeDate%}
    	//开始日期
    	if($scope.dt.from.date != ""  && $.type($scope.dt.from.date)=='date'){
    		var fromYear = $scope.dt.from.date.getFullYear() + "";
    		var fromMonth = $scope.dt.from.date.getMonth() + 1 + "";
    		var fromDate = $scope.dt.from.date.getDate() + "";
    		...
    		$scope.params.startTime = fromYear + fromMonth + fromDate;
    	} else {
    		$scope.params.startTime = null;
    	}
    	//结束日期
    	if($scope.dt.to.date != ""  && $.type($scope.dt.to.date)=='date'){
    		var toYear = $scope.dt.to.date.getFullYear() + "";
    		var toMonth = $scope.dt.to.date.getMonth() + 1 + "";
    		var toDate = $scope.dt.to.date.getDate() + "";
    		if(toMonth.length == 1){
    			toMonth = "0" + toMonth;
    		}
    		if(toDate.length == 1){
    		...
    		$scope.params.endTime = toYear + toMonth + toDate;
    	} else {
        ...
        $http.post('/dep-manage/{%queryUrl%}', $.param($scope.params), {
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
            }
        }).success(function(data) {
            ...
            sessionStorage.{%controllerName%}Params = JSON.stringify($scope.params)
        }).error(function(data, status) {
        	$scope.queryButtonStatus = false;
            if (status == 401) {
                tokenFailure($cookies, $location, $timeout, hints);
    ...
    $scope.download = function() {
    	...
    	window.location = protocol + '//' + host + '/dep-manage/{%downloadUrl%}?'+$.param($scope.params);
        $timeout(function(){
